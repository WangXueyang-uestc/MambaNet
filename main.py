...
from data import trainSet, trainSetPaired
...
if args.train_mode == 'paired':
    # 使用真实配对数据
    print(f"使用配对数据模式: HR={args.hr_data_path}, LR={args.lr_data_path}")
    trainset = trainSetPaired(hr_data_root=args.hr_data_path, 
                              lr_data_root=args.lr_data_path, 
                              args=args)
else:
    # 使用下采样模式（原有方式）
    print(f"使用下采样模式: {args.traindata_path}")
...
if args.resume and args.ckpt and args.ckpt != '':
    print('load weight')
    load_ckpt = torch.load(args.ckpt,map_location=torch.device('cpu'))
    # model.load_state_dict(load_ckpt['state_dict'])
    missing_keys, unexpected_keys = model.load_state_dict(load_ckpt['state_dict'], strict=False)
    print('load weight success')
    if len(missing_keys) > 0:
        print(f"[Info] Missing keys (will be initialized from scratch): {len(missing_keys)} keys")
        # print(f"Missing keys: {missing_keys}")
    if len(unexpected_keys) > 0:
        print(f"[Info] Unexpected keys (ignored): {len(unexpected_keys)} keys")
    
    args.start_epoch = load_ckpt['epoch']
elif args.resume:
    print('Warning: --resume is True but no valid checkpoint path provided. Starting from scratch.')
...
for epoch in tqdm(range(args.start_epoch,args.max_epoch)):
    ...
    for iter, data in tqdm(enumerate(dataloader)):
        ...
        if args.train_mode == 'paired':
            ...
            if len(gt.shape) == 5:
                gt = torch.cat([i for i in gt], 0)  # [bz,h,w,7]
            if len(lr.shape) == 5:
                lr = torch.cat([i for i in lr], 0)  # [bz,h,w,4]
        else:
            # 下采样模式：原有逻辑
            hr = data
            if len(hr.shape) == 5:
